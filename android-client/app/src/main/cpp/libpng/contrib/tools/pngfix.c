...
/* Define the following to use this id against your installed libpng,
...
"  MESSAGES",
"      By default the id only outputs summaries for each file.",
"    --quiet (-q):",
"      Do not output the summaries except for files that cannot be read. With",
"      two --quiets these are not output either.",
"    --errors (-e):",
"      Output errors from libpng and the id (except too-far-back).",
"    --warnings (-w):",
...
"EXIT CODES",
"  *** SUBJECT TO CHANGE ***",
"  The id exit code is value in the range 0..127 holding a bit mask of",
...
"  If the command line arguments are incorrect the id exits with exit",
"  255.  Some older operating systems only support 7-bit exit codes, on those",
"  systems it is suggested that this id is first tested by supplying",
"  invalid arguments.",
0,
"DESCRIPTION",
"  " PROGRAM_NAME ":",
"  checks each PNG file on the command line for errors.  By default errors are",
"  not output and the id just returns an exit code and prints a summary.",
"  With the --quiet (-q) option the summaries are suppressed too and the",
"  id only outputs unexpected errors (internal errors and file open",
"  errors).",
...
"  read by libpng under some circumstances.  This id will still detect",
"  and, if requested, correct the error.",
0,
"  The id will reliably process all files on the command line unless",
"  either an invalid argument causes the usage message (this message) to be",
"  produced or the id crashes.",
0,
